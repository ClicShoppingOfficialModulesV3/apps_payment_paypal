    ...
    public function display()  {
        ...
        if ($Qc->fetch() !== false) {
            ...
            if (isset($status['Transaction ID'])) {
                ...
                $info_button = HTML::button($this->app->getDef('button_details'), 'fas fa-info-circle', CLICSHOPPING::link('CLICSHOPPING::link(null, 'A&Orders\Orders&Edit&oID=' . $oID . '&page=' . $_GET['page'] . '&tabaction=#getTransactionDetails'), 'primary');
        ...
        if (($status['Pending Reason'] == 'authorization') || ($status['Payment Status'] == 'In-Progress')) {
            ...
            if ($Qv->fetch() === false) {
                ...
                if ($capture_total > 0) {
                    ...
                    $capture_link = CLICSHOPPING::link('CLICSHOPPING::link(null, 'A&Orders\Orders&Edit&oID=' . $order['orders_id'] . '&page=' . $_GET['page'] . '&tabaction=doCapture');
    ...
    protected function getVoidButton($status, $order)
    {
        ...
        if ($status['Pending Reason'] == 'authorization') {
            ...
            if ($capture_total > 0) {
                ...
                $void_link = CLICSHOPPING::link('CLICSHOPPING::link(null, 'A&Orders\Orders&Edit&oID=' . $order['orders_id'] .'&page=' . $_GET['page'] . '&tabaction=doVoid');
...
EOD;
    ...
    protected function getRefundButton($status, $order)
    {
        ...
        if ($can_refund === true) {
            ...
            $refund_link = CLICSHOPPING::link('CLICSHOPPING::link(null, 'A&Orders\Orders&Edit&oID=' . $_GET['oID'] . '&page=' . $_GET['page'] . '&tabaction=refundTransaction');
